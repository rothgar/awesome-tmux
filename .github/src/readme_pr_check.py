#!/usr/bin/env python3
"""
PR-time check for awesome-tmux README changes.

For every PR that touches README.md:
  - Extract added GitHub URLs, look up each repo's first commit, and
    post a status table (✅/⛔) under MARKER.
  - Run an alphabetical-order check on the post-PR README and append
    the result under ALPHA_MARKER.
  - When EVERY added GitHub repo is confirmed younger than the
    minimum age (default 6 months), auto-close the PR with an
    explanatory comment.
...
MARKER = "<!-- readme-pr-info -->"
CLOSE_COMMENT_MARKER = "<!-- readme-pr-info:close -->"
ALPHA_MARKER = "<!-- readme-pr-info:alphabetical -->"
...
MIN_AGE_MONTHS = 6  # change here to use a different minimum age
...
def go_no_go_for_first_commit(iso_date_str: str) -> tuple[str, str, str]:
    """Return (display_date, emoji, status)."""
    parsed = parse_iso_date_utc(iso_date_str)
    if not parsed:
        return ("—", "⛔", STATUS_UNKNOWN)
    today_utc = datetime.now(timezone.utc).date()
    cutoff = date_months_before(today_utc, MIN_AGE_MONTHS)
    if parsed <= cutoff:
...
def parse_link_header(link_header: str) -> dict:
    ...
    for part in link_header.split(","):
        ...
        if not sections:
            continue
        if not (sections[0].startswith("<") and sections[0].endswith(">")):
            continue
        url = sections[0][1:-1]
        rel = None
        for s in sections[1:]:
...
def build_comment_table(
    readme_urls: list[str], client: GithubClient
) -> tuple[str, list[tuple[str, str, str]]]:
    """Build the marker comment body and return a parallel list of
    (repo_url, status, iso_date) for every real GitHub repo found."""
...
def build_close_comment(too_young: list[tuple[str, str]]) -> str:
    lines = [
        ...
        f"Closing this PR because every GitHub repository added by this "
        f"change is younger than the {MIN_AGE_MONTHS}-month minimum required "
        f"by this list.",
    ...
    lines += [
        "",
        f"What to do next: please reopen this PR once each of the above "
        f"repositories has been active for at least {MIN_AGE_MONTHS} months "
        f"(by first commit). The status table above will refresh "
        f"automatically when you reopen or push changes.",
    ]
    return "\n".join(lines)
...
def fetch_base_readme(client: GithubClient, pr_number: str) -> str | None:
    ...
    if not ref:
        return None
    try:
        data, _ = client.api(f"repos/{client.repository}/contents/README.md?ref={ref}")
    except SystemExit:
        return None
...
def main() -> None:
    ...
    readme_urls = find_readme_urls(changed_files if isinstance(changed_files, list) else [])
    readme_patch = ""
